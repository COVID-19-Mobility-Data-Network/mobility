...
Once the travel data has been converted the to gerenalized travel data format in the `travel_data_template` and `travel_data_sim` objects, utility functions can be used to build the data matrices required for fitting and simulating models. For fitting gravity models the data must be converted into data matrices representing travel among locations ($M$), along with distances ($D$) and population sizes ($N$). 


### Unique IDs
The first step is to add unique IDs to the travel data. If the data have all unique names at the lowest admin level then hierarchical IDs are not needed. However, if the data include repeated admin names, then higher admin levels must be included in the unique IDs.
...
Gravity models require that distances be provided for all routes even is these observations are missing in the mobility matrix ($M$). A distance matrix for all unique routes can be built by getting all unique coordinates in the travel data using the `get_unique_coords` function and then calculating the distances among all locations with `get_distance_matrix`.
...
Fitting gravity models and models of travel peobability require a vector of population size for each location that is the same length the number of rows and columns in the mobility matrix ($M$) and distance matrix ($D$).
