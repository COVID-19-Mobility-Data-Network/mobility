...
So, what do we mean by "mobility" model? The mobility model estimated by this package is different from using just a gravity model in that it estimates values along the diagonal of the mobility matrix ($\hat{M}_{ii}$), unlike gravity model formulations which estimate off-diagonal values only ($\hat{M}_{ij}$). This is achieved by combining models that estimate: 1) the probabilty of travel outside the origin location $i$ ($\tau_i$) which helps us to fill in the diagonal of the mobility matrix, and 2) the distribution of travel from the origin location $i$ to all $j$ destinations ($\pi_{ij}$) which fills in the off-diagonal elements. 
...
The prediction error results compare actual trip count values in the data matrix $M$ with the predicted values of the estimated mobility model ($\hat{M}$). These results indicate that our model has an average magnitude of error of about 447 trip counts (RMSE), which is on average about 39% of the observed trip count values. The R-squared value shows that our model captures approximately 96% of the variance observed in the supplied mobility data (a rather high value since we are fitting to simulated data).
