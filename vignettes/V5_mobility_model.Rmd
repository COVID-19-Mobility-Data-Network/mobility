...
So, what do we mean by "mobility" model? The mobility model estimated by this package is different from using just a gravity model in that it uses a flexible departure-diffusion model framework that estimates diagonal and off-diagonal elements of the mobility matrix ($\hat{M}_{ii}$) separately and combines both models using conditional probability rules. This is achieved by combining models that estimate: 1) the probabilty of travel outside the origin location $i$ ($\tau_i$)---the departure process, and 2) the distribution of travel from the origin location $i$ to all $j$ destinations ($\pi_{ij}$)---the diffusion process. 
